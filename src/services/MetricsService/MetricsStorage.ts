...
export class DefaultMetricsStorage implements MetricsStorageWriter, MetricsStorageReader {
  ...
  private deserializeMetrics(serializedMetrics: string): Metric[] {
    return serializedMetrics.split('#').map(metric => {
      const metricComponents = metric.split(';');
      const timestamp = Number(metricComponents[0]);
      const identifier = metricComponents[1];
      const region = metricComponents[2];
      const payload: [string, string][] = JSON.parse(metricComponents[3]);
      return new Metric(timestamp, identifier, region, payload);
    });
  }
}
